...
// petunindan tad
void button_wait_combi (edict_t *self)
{
...
void button_wait_combi7 (edict_t *self)
{
...
void button_wait_combi2 (edict_t *self)
{
...
void button_wait_combi5 (edict_t *self)
{
...
void button_wait_combi3 (edict_t *self)
{
...
void button_wait_combi8 (edict_t *self)
{
...
void button_wait_combi9 (edict_t *self)
{
...
void button_wait_combi_wrong (edict_t *self)
{
...
// petunindan tad
void button_fire_combi7 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi7);
}

void button_fire_combi2 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi2);
}

void button_fire_combi5 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi5);
}

void button_fire_combi3 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi3);
}

void button_fire_combi8 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi8);
}

void button_fire_combi9 (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi9);
}

void button_fire_combi_wrong (edict_t *self)
{
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
	...
	Move_Calc (self, self->moveinfo.end_origin, button_wait_combi_wrong);
}
//
...
extern void monster_fire_gladlaser (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int flashtype);

void button_killed_combi (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	self->activator = attacker;
	self->health = self->max_health;
	self->takedamage = DAMAGE_NO;
	
	//button sequence start
	if (self->speed == 7)
	{
		button_fire_combi7(self);
		gi.centerprintf(attacker, "ok so far...");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else if ((self->speed == 2) && (sevenpressed == 1))
	{
		button_fire_combi2(self);
		gi.centerprintf(attacker, "ok so far...");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else if ((self->speed == 5) && (twopressed == 1))
	{
		button_fire_combi5(self);
		gi.centerprintf(attacker, "ok so far...");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else if ((self->speed == 3) && (fivepressed == 1))
	{
		button_fire_combi3(self);
		gi.centerprintf(attacker, "ok so far...");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else if ((self->speed == 8) && (threepressed == 1))
	{
		button_fire_combi8(self);
		gi.centerprintf(attacker, "ok so far...");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else if ((self->speed == 9) && (eightpressed == 1))
	{
		button_fire_combi9(self);
		gi.centerprintf(attacker, "deactivation code recognized");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("misc/talk1.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		button_fire_combi_wrong(self);
		gi.centerprintf(attacker, "Security breach!");
		gi.sound (attacker, CHAN_AUTO, gi.soundindex ("world/x_alarm.wav"), 1, ATTN_NORM, 0);
		/* BANG ! and show the clients */
		T_RadiusDamage (attacker, attacker, 3000, NULL, 5, MOD_FEEDBACKLASER);
...
// petunindan tad
// tad final numeric buttons
void SP_func_combi_button (edict_t *ent)
{
	vec3_t	abs_movedir;
	float	dist;
