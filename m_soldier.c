...
added "Screen Soldier"
changed skinnum "6*self->" to "8*self->"
changed "s.skinnum % 6" to "s.skinnum % 8"
26.05.2021
***
...
{
	if ( ((self->s.skinnum % 8) <= 1) && (self->enemy) && visible(self, self->enemy))
	{
		soldier_fire(self, 0);
	}
}
...
void soldier_fire (edict_t *self, int in_flash_number)
{
	...
	if ((self->s.skinnum % 8) < 2)
		flash_index = blaster_flash[flash_number];
	else if ((self->s.skinnum % 8) < 4)
...
//				gi.dprintf ("infantry shot aborted due to bad target\n");
	...
	// petunindan soldier_sc mgun
	else if ((self->s.skinnum % 8) <= 9)
	{
		// PMM - changed to wait from pausetime to not interfere with dodge code
		if (!(self->monsterinfo.aiflags & AI_HOLD_FRAME))
			self->wait = level.time + (3 + rand() % 8) * FRAMETIME;
...
//	if ((range(self, self->enemy) >= RANGE_MID) && (r < (skill->value*0.25) && ((self->s.skinnum % 8) <= 3)))
...
//	PMM - don't let machinegunners run & shoot
		if ((random() > 0.75) && ((self->s.skinnum % 8) <= 3))
		{
...
void soldierh_attack2_refire2 (edict_t *self)
{
	...
	if ( ((skill->value == 3) && (random() < 0.5))
		|| (range(self, self->enemy) == RANGE_MELEE) && (self->s.skinnum % 8) < 4)
		self->monsterinfo.nextframe = FRAME_attak204;
}
...
void SP_monster_soldier_ss (edict_t *self)
{
	...
	SP_monster_soldier_x (self);
	// Lazarus: custom skins
	self->s.skinnum = 4 + 8*self->style;
}

// petunindan soldier_sc
void SP_monster_soldier_screen (edict_t *self)
{
	...
	sound_death_ss = gi.soundindex ("soldier/soldeth3.wav");
	gi.soundindex ("soldier/solatck3.wav");
	self->classname = "monster_soldier_screen";
	self->common_name = "Screen Soldier";
	self->monsterinfo.power_armor_type = POWER_ARMOR_SCREEN;
