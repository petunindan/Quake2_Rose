...
void ParasiteGrenade (edict_t *self)
{
	vec3_t	start;
...
//	VectorMA (aim, pitch, up, aim);
	...
	fire_roseprox_monster (self, start, aim, 1, 600, flash_number);
}
...
// petunindan tad
mframe_t parasite_frames_prox [] =
{
	ai_charge, 0,	ParasiteGrenade,
	ai_charge, 0,	NULL,
};
mmove_t parasite_move_prox = {39, 40, parasite_frames_prox, parasite_start_run};
//
...
// petunindan tad
void parasite_attack_prox (edict_t *self)
{

	if (random() <= 0.2)
		self->monsterinfo.currentmove = &parasite_move_prox;
	else
		self->monsterinfo.currentmove = &parasite_move_drain;
}
...
void SP_monster_parasite_prox (edict_t *self)
{
	...
	self->monsterinfo.walk = parasite_start_walk;
	self->monsterinfo.run = parasite_start_run;
	self->monsterinfo.attack = parasite_attack_prox;
	self->monsterinfo.sight = parasite_sight;
	self->monsterinfo.search = parasite_search; //Knightmare- this was missing
	self->monsterinfo.idle = parasite_idle;
	self->monsterinfo.blocked = parasite_blocked;		// PGM
