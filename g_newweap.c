...
void Prox_Explode_Monster (edict_t *ent)
{
	vec3_t		origin;
	edict_t		*owner;
...
// petunindan rose prox
void Prox_Field_Touch_Rose (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	edict_t *prox;
...
//			gi.dprintf ("%f - prox already gone off!\n", level.time);
	...
	if(prox->teamchain == ent)
	{
		gi.sound (ent, CHAN_VOICE, gi.soundindex ("weapons/proxgo.wav"), 1, ATTN_NORM, 0);
		prox->think = Prox_Explode;
		prox->nextthink = level.time + PROX_TIME_DELAY;
		return;
	}
...
// petunindan rose proximity open
void prox_open_rose (edict_t *ent)
{
	edict_t *search;
...
//	gi.dprintf("%f\n", ent->velocity[2]);
	if(ent->s.frame == 9)	// end of opening animation
	{
		...
		//ent->owner = NULL;
		if(ent->teamchain)
			ent->teamchain->touch = Prox_Field_Touch_Rose;
		while ((search = findradius(search, ent->s.origin, prox_radius->value + 10)) != NULL)
		{
			if (!search->classname)			// tag token and other weird shit
				continue;
...
// petunindan prox land rose
#define STOP_EPSILON_ROSE	0.1
void prox_land_rose (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	edict_t	*field;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
//				gi.dprintf ("bad normal for surface, exploding!\n");
		...
		for (i=0 ; i<3 ; i++)
		{
			change = plane->normal[i]*backoff;
			out[i] = ent->velocity[i] - change;
			if (out[i] > -STOP_EPSILON_ROSE && out[i] < STOP_EPSILON_ROSE)
				out[i] = 0;
		}
		if ((out[2] > 60) && !havehost) // always open up if attached to host
			return;
...
// petunindan rose prox
void fire_roseprox (edict_t *self, vec3_t start, vec3_t aimdir, int damage_multiplier, int speed)
{
	edict_t	*prox;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
//		gi.dprintf ("start %s    aim %s   speed %d\n", vtos(start), vtos(aimdir), speed);
	...
	//FIXME - this needs to be bigger.  Has other effects, though.  Maybe have to change origin to compensate
	// so it sinks in correctly.  Also in lavacheck, might have to up the distance
	//VectorSet (prox->mins, -2, -2, -2);
	//VectorSet (prox->maxs, 2, 2, 2);
	prox->s.modelindex = gi.modelindex ("models/objects/grenade_rose/tris.md2");
	prox->owner = self;
	prox->teammaster = self;
	prox->touch = prox_land_rose;
//	prox->nextthink = level.time + prox_life->value;
	prox->think = Prox_Explode;
	prox->dmg = prox_damage->value*damage_multiplier;
	prox->classname = "prox";
...
// prox with grenade class to AI ivade
void fire_roseprox_monster (edict_t *self, vec3_t start, vec3_t aimdir, int damage_multiplier, int speed)
{
	edict_t	*prox;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
//		gi.dprintf ("start %s    aim %s   speed %d\n", vtos(start), vtos(aimdir), speed);
	...
	prox->touch = prox_land_rose;
//	prox->nextthink = level.time + prox_life->value;
	prox->think = Prox_Explode_Monster;
	prox->dmg = prox_damage->value*damage_multiplier;
	prox->classname = "grenade";
	prox->svflags |= SVF_DAMAGEABLE;
	prox->flags |= FL_MECHANICAL;
