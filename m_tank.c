...
// petunindan rose
void deathtank_refire_rocket (edict_t *self);
void deathtank_doattack_rocket (edict_t *self);
void deathtank_reattack_blaster (edict_t *self);
...
static int	sound_tanklaser;
static int	sound_tankplasma;
...
// petunindan tad
void DeathTankBlaster (edict_t *self)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	end;
	...
	//monster_fire_blaster (self, start, dir, 30, 800, flash_number, EF_BLASTER, BLASTER_ORANGE);
	gi.sound (self, CHAN_WEAPON, sound_tanklaser, 1, ATTN_NORM, 0);
...
// petunindan tad
void DeathTankRocket (edict_t *self)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	dir;
...
//			gi.multicast (start, MULTICAST_ALL);
	...
	if (blindfire)
	{
		...
		if (!(trace.startsolid || trace.allsolid || (trace.fraction < 0.5)))
		{
			gi.sound (self, CHAN_WEAPON, sound_tankplasma, 1, ATTN_NORM, 0);
			fire_tadplasma (self, start, dir, 50, 725, 60, 60);
		}
		else 
		{
			// try shifting the target to the left a little (to help counter large offset)
...
mframe_t deathtank_frames_attack_blast [] =
{
	ai_charge, 0,	NULL,
	ai_charge, 0,	NULL,
	ai_charge, 0,	NULL,
	...
	ai_charge, -1,	NULL,
	ai_charge, 0,	NULL,
	ai_charge, 0,	DeathTankBlaster,		// 10
	ai_charge, 0,	NULL,
	ai_charge, 0,	NULL,
	ai_charge, 0,	DeathTankBlaster,
	ai_charge, 0,	NULL,
	ai_charge, 0,	NULL,
	ai_charge, 0,	DeathTankBlaster			// 16
};
mmove_t deathtank_move_attack_blast = {FRAME_attak101, FRAME_attak116, deathtank_frames_attack_blast, deathtank_reattack_blaster};

mframe_t deathtank_frames_reattack_blast [] =
{
	ai_charge, 0,	NULL,
	ai_charge, 0,	NULL,
	ai_charge, 0,	DeathTankBlaster,
...
mmove_t deathtank_move_reattack_blast = {FRAME_attak111, FRAME_attak116, deathtank_frames_reattack_blast, deathtank_reattack_blaster};
...
// petunindan tad
mframe_t deathtank_frames_attack_post_blast [] =	
{
	ai_move, 0,		NULL,				// 17
	ai_move, 0,		NULL,
	ai_move, 2,		NULL,
...
mmove_t deathtank_move_attack_post_blast = {FRAME_attak117, FRAME_attak122, deathtank_frames_attack_post_blast, tank_run};
//
	...
	if (skill->value >= 2)
		if (visible (self, self->enemy))
			if (self->enemy->health > 0)
				if (random() <= 0.6)
				{
					self->monsterinfo.currentmove = &deathtank_move_reattack_blast;
					return;
				}
	self->monsterinfo.currentmove = &deathtank_move_attack_post_blast;
}
//
...
// petunindan tad
mframe_t deathtank_frames_attack_pre_rocket [] =
{
	ai_charge, 0,  NULL,
	ai_charge, 0,  NULL,
	ai_charge, 0,  NULL,
...
mmove_t deathtank_move_attack_pre_rocket = {FRAME_attak301, FRAME_attak321, deathtank_doattack_rocket}; // deathtank_frames_attack_pre_rocket

mframe_t deathtank_frames_attack_fire_rocket [] =
{
	ai_charge, -3, NULL,			// Loop Start	22 
	ai_charge, 0,  NULL,
	ai_charge, 0,  DeathTankRocket,		// 24
	ai_charge, 0,  NULL,
	ai_charge, 0,  NULL,
	ai_charge, 0,  DeathTankRocket,
	ai_charge, 0,  NULL,
	ai_charge, 0,  NULL,
	ai_charge, -1, DeathTankRocket		// 30	Loop End
};
mmove_t deathtank_move_attack_fire_rocket = {FRAME_attak322, FRAME_attak330, deathtank_frames_attack_fire_rocket, deathtank_refire_rocket};
//
	...
	if ( skill->value >= 2 )
		if (self->enemy->health > 0)
			if (visible(self, self->enemy) )
				if (random() <= 0.4)
				{
					self->monsterinfo.currentmove = &deathtank_move_attack_fire_rocket;
					return;
				}
	self->monsterinfo.currentmove = &tank_move_attack_post_rocket;
}
...
// petunindan tad
void deathtank_attack(edict_t *self)
{
	vec3_t	vec;
	float	range;
	float	r;
...
//				gi.dprintf ("blindfire - NO SHOT\n");
	...
	{
		if (r < 0.4)
			self->monsterinfo.currentmove = &deathtank_move_attack_blast;
		else 
			self->monsterinfo.currentmove = &deathtank_move_attack_blast;
	}
	else if (range <= 250)
...
// petunindan tank general

void SP_monster_tank_general (edict_t *self)
{
	if (deathmatch->value)
	{
		G_FreeEdict (self);
	...
	self->classname = "monster_tank_general";
	self->common_name = "Tank General";
...
void SP_monster_death_tank (edict_t *self)
{
	if (deathmatch->value)
	{
		G_FreeEdict (self);
	...
	sound_tanklaser = gi.soundindex ("tank/tnkatck3.wav");
	sound_tankplasma = gi.soundindex ("tank/tnkatck1.wav");

	// Tank General

	self->classname = "monster_death_tank";
	self->common_name = "Death Tank";
	...
	self->monsterinfo.run = tank_run;
	self->monsterinfo.dodge = NULL;
	self->monsterinfo.attack = deathtank_attack;
	self->monsterinfo.melee = NULL;
	self->monsterinfo.sight = tank_sight;
	self->monsterinfo.idle = tank_idle;
	self->monsterinfo.blocked = tank_blocked;		// PGM
