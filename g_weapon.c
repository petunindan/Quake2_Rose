...
/*
=================
petunindan rose fire_gladlaser
=================
*/
...
// petunindan tad
void tadplasma_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t		origin;
	...
	if (other->takedamage)
	{
		T_Damage (other, self, self->owner, self->velocity, self->s.origin, plane->normal, self->dmg, 0, 0, MOD_PHALANX);
	}
	
	T_RadiusDamage(self, self->owner, self->radius_dmg, other, self->dmg_radius, MOD_PHALANX_SPLASH);
