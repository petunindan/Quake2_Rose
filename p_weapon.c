...
// PGM
	switch(ent->client->pers.weapon->tag)
	{
		...
		case AMMO_TESLA:
			...
			break;
		case AMMO_PROXIMITY_GRENADES:
			fire_roseprox (ent, start, forward, damage_multiplier, speed);
			break;
		default:
...
// petunindan rose prox grenader
void Weapon_Proximity_Grenade (edict_t *ent)
{
	static int	pause_frames[]	= {29,34,39,48,0};
...
// petunindan rose
// code from stroggdms's p_weapon.c
...
// petunindan rose laser

#define	LOVERLOAD_DAMAGE			300
#define	LOVERLOAD_DAMAGE_RADIUS	5 // Quake Units
#define LOVERLOAD_BLOW_TIME		 50 // 1/10 seconds

void		Start_Loverload_Mode(edict_t *the_doomed_one); // setup and start self destruct mode

qboolean Loverload_Active(edict_t *the_doomed_one);

void Loverload_Explode(edict_t *the_doomed_one);

void Loverload_Cancel(edict_t *the_spared_one);

void Laser_Explode(edict_t *the_doomed_one)
{
	/* BANG ! and show the clients */
	T_RadiusDamage (the_doomed_one, the_doomed_one, LOVERLOAD_DAMAGE, NULL, LOVERLOAD_DAMAGE_RADIUS, MOD_FEEDBACKLASER);
...
=================
*/
void fire_roselaser (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	vec3_t		from;
	vec3_t		end;
	trace_t		tr;
...
void weapon_laser_fire (edict_t *ent)
{
	...
	if (ent->client->buttons & BUTTON_ATTACK)//Will: Added for the sonic rail, it fires when released
	{
		
		// starting countdown
		
		if (ent->SonicDamage <= 15)
		{
		gi.centerprintf (ent, "6%\n");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		gi.sound( ent, CHAN_WEAPON, gi.soundindex("weapons/laschrg.wav"), 1, ATTN_NONE, 0 );
		}
	
		//
		
		ent->SonicDamage += 30;//Charging Rate: increases the integer by 5 every frame that the attack key is held down

		//if (ent->SonicDamage>600)
		//ent->SonicDamage= 600; //This is the maximum damage of the Sonic Rail
	
		// countdownd
	
		if (ent->SonicDamage == 30)
		{
		gi.centerprintf (ent, "12%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 60)
		{
		gi.centerprintf (ent, "18%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 90)
		{
		gi.centerprintf (ent, "24%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
	
		if (ent->SonicDamage == 120)
		{
		gi.centerprintf (ent, "30%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 150)
		{
		gi.centerprintf (ent, "36%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 180)
		{
		gi.centerprintf (ent, "42%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 210)
		{
		gi.centerprintf (ent, "48%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 240)
		{
		gi.centerprintf (ent, "54%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 270)
		{
		gi.centerprintf (ent, "60%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 300)
		{
		gi.centerprintf (ent, "66%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 330)
		{
		gi.centerprintf (ent, "72%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 360)
		{
		gi.centerprintf (ent, "78%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 390)
		{
		gi.centerprintf (ent, "84%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 420)
		{
		gi.centerprintf (ent, "92%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 450)
		{
		gi.centerprintf (ent, "98%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
	
		if (ent->SonicDamage == 480)
		{
		gi.centerprintf (ent, "102%%");
		ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
		...
		if (ent->SonicDamage > 480)
		{
			gi.centerprintf (ent, "Overload!");
			gi.sound( ent, CHAN_WEAPON, gi.soundindex("world/x_alarm.wav"), 1, ATTN_NONE, 0 );
			ent->client->pers.inventory[ent->client->ammo_index]-= 3;
		}
		...
		if (ent->client->pers.inventory[ent->client->ammo_index] < 1)
		{
			...
			VectorSet(offset, 0, 7, ent->viewheight-8);
			P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
			//fire_rail (ent, start, forward, ent->SonicDamage, 0);//new fire_rail function
			// send muzzle flash
			//gi.WriteByte (svc_muzzleflash);
			//gi.WriteShort (ent-g_edicts);
			//gi.WriteByte (MZ_RAILGUN | is_silenced);
			...
			fire_roselaser (ent, start, forward, ent->SonicDamage, 2);
			...
			if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
			ent->client->pers.inventory[ent->client->ammo_index]--;
			ent->SonicDamage =0; //Will: Sets the integer back to 0
			
			NoAmmoWeaponChange (ent);
		}
	
	// gun overload
		if (ent->SonicDamage > 550)
		{
			/* BANG ! and show the clients */
			T_RadiusDamage (owner, owner, 3000, NULL, 5, MOD_FEEDBACKLASER);
			gi.WriteByte (svc_temp_entity);
			gi.WriteByte (TE_EXPLOSION1);
	...
	if (ent->SonicDamage<36)
		ent->SonicDamage= 6; //This sets the minimum damage of the Sonic Rail
		//Will: end of section
...
{
	static int	pause_frames[]	= {47, 0};
	static int	fire_frames[]	= {4, 0};

	Weapon_Generic (ent, 3, 9, 47, 52, pause_frames, fire_frames, weapon_laser_fire);
	if (is_quadfire)
		Weapon_Generic (ent, 3, 9, 47, 52, pause_frames, fire_frames, weapon_laser_fire);
}
...
// petunindan tad
void weapon_plasmagun_fire (edict_t *ent, qboolean altfire)
{
	vec3_t		start;
	vec3_t		forward, right, up;
	vec3_t		offset;
	...
	// new plasmagun damage
	damage = 35 + (int)(random() * phalanx_damage2->value);
	radius_damage = 35;
	damage_radius = phalanx_radius->value;
	...
	if (ent->client->ps.gunframe == 8)
	{
		...
		fire_tadplasma (ent, start, forward, damage, phalanx_speed->value, damage_radius, radius_damage);
...
{
	static int	pause_frames[]	= {46, 0};
	static int	fire_frames[]	= {4, 6, 0};

	Weapon_Generic (ent, 3, 6, 46, 51, pause_frames, fire_frames, weapon_plasmagun_fire);
